
//  Product
            export const Products = [
                {   productId: 1,
                    addedBy: 'elesh',
                    productName: 'African Drum',
                    price: 10000,
                    condition: 'new',
                    deliver: 'pickup-only',
                    src: '/images/products/african_drum.jpg',
                    numOfItemsSold: 8,
                    description: 'Experience the soul-stirring rhythm of Africa with our authentic African Drum, handcrafted by skilled artisans using premium materials. Each drum carries the rich heritage and vibrant energy of African culture, perfect for musical performances, cultural celebrations, or as a stunning decorative piece.',
                    quantity: 0,
                    stock: 1,
                    star: 3,
                    totalVotes: 200,
                    isAdded: false,
                    size:'',
                    orderStatus:['processing', 'shipped', 'completed'],
                    categories: ['new', 'art', 'trending'],
                    productPageVisits: 235
            
                },
                {   productId: 2,
                    addedBy: 'elesh',
                    productName: 'Dope Chick red heels',
                    price: 12000,
                    condition: 'new',
                    deliver: 'yes',
                    src: '/images/products/women/shoes/freaky_heels.png',
                    quantity: 0,
                    stock: 1,
                    description: 'Step into confidence with Dope Chick, a vibrant African-inspired clothing line designed for the modern woman who embraces her roots with style. Each piece is a celebration of African heritage, crafted with bold prints, luxurious fabrics, and contemporary flair to make you stand out effortlessly.',
                    star: 4,
                    totalVotes: 8,
                    numOfItemsSold: 10,
                    isAdded: false,
                    size:'',
                    orderStatus:['processing', 'shipped', 'completed'],
                    categories: ['new', 'women shoes', 'trending'],
                    productPageVisits: 422
            
                },
                {   productId: 3,
                    addedBy: 'elesh',
                    addedBy: 'elesh',
                    productName: 'Senator Bassey',
                    price: 8000,
                    condition: 'new',
                    deliver: 'yes',
                    src: '/images/products/men/clothes/senator_bassey.jpg',
                    quantity: 0,
                    stock: 1,
                    description: 'Command respect and exude sophistication with Senator Bassey, a premium African menswear collection that blends traditional craftsmanship with modern refinement. Inspired by the dignified style of African leaders, each piece is meticulously tailored for the man who carries himself with authority and cultural pride.',
                    star: 5,
                    totalVotes: 7,
                    numOfItemsSold: 3,
                    isAdded: false,
                    size:'',
                    orderStatus:['processing', 'shipped', 'completed'],
                    categories: ['trending',  'men clothings', 'agbada', 'men natives'],
                    productPageVisits: 334
            
                },
                {   productId: 4,
                    addedBy: 'elesh',
                    productName: 'Igara Chicken',
                    price: 5000,
                    condition: 'new',
                    deliver: 'pickup-only',
                    src: '/images/products/men/clothes/men_lace.jpg',
                    quantity: 0,
                    stock: 1,
                    description: 'Feast your eyes on Igara Chicken, the daring new African menswear line that blends streetwise swagger with traditional flair. Inspired by the fiery spirit of West African pepper chicken, this collection serves up looks as bold as the dish itself. Perfect for men who are not afraid to stand out.',
                    star: 5,
                    totalVotes: 5,
                    numOfItemsSold: 5,
                    isAdded: false,
                    size:'',
                    orderStatus:['processing', 'shipped', 'completed'],
                    categories: ['trending', 'men clothings', 'agbada', 'men natives'],
                    productPageVisits: 599
            
                },
                {   productId: 5,
                    addedBy: 'elesh',
                    productName: 'Sexy Red Flamingo',
                    condition: 'used',
                    deliver: 'yes',
                    price: 6000,
                    src: '/images/products/women/shoes/sexy_red_flamingo.png',
                    quantity: 0,
                    stock: 1,
                    description: "Slay every stride in Sexy Red Flamingo, the head-turning African-inspired heels that blend fierce femininity with unapologetic glamour. Like the vibrant flamingos of Africa's salt lakes, these statement shoes make you impossible to ignore.",
                    star: 5,
                    totalVotes: 5,
                    numOfItemsSold: 6,
                    isAdded: false,
                    size:'',
                    orderStatus:['processing', 'shipped', 'completed'],
                    categories:  ['new', 'women shoes', 'bestseller'],
                    productPageVisits: 157
            
                },
                {   productId: 6,
                    addedBy: 'elesh',
                    productName: 'Ankara Shanti',
                    price: 20000,
                    condition: 'new',
                    deliver: 'pickup-only',
                    src: '/images/products/women/clothes/ankara.jpg',
                    quantity: 0,
                    stock: 1,
                    description: '',
                    star: 5,
                    totalVotes: 5,
                    numOfItemsSold: 4,
                    isAdded: false,
                    size:'',
                    orderStatus:['processing', 'shipped', 'completed'],
                    categories: ['bestseller', 'women clothings', 'women natives', 'women ankara'],
                    productPageVisits: 267
            
                },
                {   productId: 7,
                    addedBy: 'elesh',
                    productName: 'Woman Bead',
                    price: 10000,
                    condition: 'used',
                    deliver: 'yes',
                    src: '/images/products/beads.jpg',
                    quantity: 0,
                    stock: 1,
                    description: '',
                    star: 5,
                    totalVotes: 2,
                    numOfItemsSold: 9,
                    isAdded: false,
                    size:'',
                    orderStatus:['processing', 'shipped', 'completed'],
                    categories: ['bestseller', 'women accessories'],
                    productPageVisits: 345
            
                },
                {   productId: 8,
                    addedBy: 'elesh',
                    productName: 'Woman Portrait by Fikor',
                    price: 7000,
                    condition: 'new',
                    deliver: 'pickup-only',
                    src: '/images/products/woman_portrait.jpg',
                    quantity: 0,
                    stock: 1,
                    description: '',
                    star: 5,
                    totalVotes: 1,
                    numOfItemsSold: 8,
                    isAdded: false,
                    size:'',
                    orderStatus:['processing', 'shipped', 'completed'],
                    categories: ['bestseller', 'art'],
                    productPageVisits: 432
            
                },
                {   productId: 9,
                    addedBy: 'elesh',
                    productName: 'Goat',
                    price: 4000,
                    condition: 'new',
                    deliver: 'yes',
                    src: '/images/products/animals/goat.jpg',
                    quantity: 0,
                    stock: 1,
                    description: '',
                    star: 5,
                    totalVotes: 9,
                    numOfItemsSold: 8,
                    isAdded: false,
                    size:'',
                    orderStatus:['processing', 'shipped', 'completed'],
                    categories: [ 'livestock & animal'],
                    productPageVisits: 236
            
                },
                {   productId: 10,
                    addedBy: 'elesh',
                    productName: 'African Wig',
                    price: 3000,
                    condition: 'new',
                    deliver: 'pickup-only',
                    src: '/images/products/women/hairs/afro_wig.jpg',
                    quantity: 1,
                    stock: 1,
                    description: '',
                    star: 5,
                    totalVotes: 11,
                    numOfItemsSold: 7,
                    isAdded: false,
                    size:'',
                    orderStatus:['processing', 'shipped', 'completed'],
                    categories: [ 'women accessories'],
                    categories: 117
            
                },
                {   productId: 11,
                    addedBy: 'elesh',
                    productName: 'Chima red shoe',
                    price: 15000,
                    condition: 'new',
                    deliver: 'yes',
                    src: '/images/products/women/shoes/sexy_red_flamingo.png',
                    quantity: 1,
                    stock: 1,
                    description: '',
                    star: 5,
                    totalVotes: 5,
                    numOfItemsSold: 5,
                    isAdded: false,
                    size:'',
                    orderStatus:['processing', 'shipped', 'completed'],
                    categories: [ 'bestseller', 'women shoes'],
                    productPageVisits: 256
            
                },
                {   productId: 12,
                    addedBy: 'elesh',
                    productName: 'Red horse shoes',
                    price: 5000,
                    condition: 'used',
                    deliver: 'yes',
                    src: '/images/products/women/shoes/sexy_red_flamingo.png',
                    quantity: 1,
                    stock: 1,
                    description: '',
                    star: 5,
                    totalVotes: 5,
                    numOfItemsSold: 10,
                    isAdded: false,
                    size:'',
                    orderStatus:['processing', 'shipped', 'completed'],
                    categories: ['new', 'women shoes'],
                    productPageVisits: 178
            
                },
            ]